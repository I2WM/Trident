...
# 需要实现测试时数据增广(test time augmentation,TTA)--trick + Test-time Local Converter
import argparse
import cv2
import glob
import numpy as np
...
from hat.archs.hat_arch import HATLocal

"""
python infer_tlc.py --output results/HAT-L_pretrain_TLC --model_path experiments/pretrained_models/HAT-L_SRx4_ImageNet-pretrain.pth
python infer.py --output results/HAT-L_pretrain_DIV2K --model_path experiments/pretrained_models/HAT-L_SRx4_ImageNet-pretrain.pth \
...
def main():
    ...
    parser.add_argument('--output', type=str, default='results/infer/01tta', help='output folder')
    parser.add_argument(
        '--task',
        type=str,
    ...
    parser.add_argument(
        '--model_path',
        type=str,
        default='./experiments/My_train_HAT-L_SRx4_DF2K_from_scratch_L1/models/net_g_800000.pth')
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(args.input, '*')))):
        ...
        if args.tta:
            ...
            cv2.imwrite(os.path.join(args.output, f'{imgname}.png'), output_tta)
